<template>
  <table class="table">
    <thead>
      <tr>
        <th v-for="field in fields" :key="field.text" class="">
          {{ field.text }}
        </th>
        <th class="">Action</th>
      </tr>
    ...
    <tbody>
      <tr v-for="element in data" :key="element.id">
        ...
        >
          <slot :name="field" :[field]="element[field]"> {{ element[field] }}</slot>
        </td>

        <td class="table__table-data table__table-data--group-buttons">
          <button
            v-if="action.view.isShow"
            class="table-data__btn table-data__btn--view"
          >
            {{ action.view.text }}
          </button>
          ...
          <router-link
            :to="{
              name: action.edit.routeName,
              params: { id: action.edit.field },
            }"
          >
            <button
              v-if="action.edit.isShow"
              class="table-data__btn table-data__btn--edit"
            >
              {{ action.edit.text }}
            </button>
          </router-link>
